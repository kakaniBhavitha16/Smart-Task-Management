...
export default function Tasks() {
  ...
  const fetchTasks = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://smart-task-management.onrender.com/api/tasks?limit=1000`);
  ...
  const handleDeleteTask = async (taskId) => {
    if (window.confirm('Are you sure you want to delete this task?')) {
      try {
        const response = await axios.delete(`https://smart-task-management.onrender.com/api/tasks/${taskId}`);
        if (response.data.success) {
  ...
  const handleToggleStatus = async (taskId, currentStatus) => {
    ...
    try {
      const response = await axios.put(`https://smart-task-management.onrender.com/api/tasks/${taskId}`, { status: newStatus });
      if (response.data.success) {
